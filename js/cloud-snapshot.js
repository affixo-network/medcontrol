(function () {
  ...
  async function createCloudSnapshot(options = {}) {
    ...
    if (!options.allowEmpty && !meaningfulState(state)) {
      return { ok: false, skipped: true, reason: 'empty_local_state' };
    }
    ...
    if (latest && JSON.stringify(latest.state) === serialized) {
      return { ok: true, skipped: true, reason: 'unchanged', snapshot: latest };
    }
  ...
  async function getLatestCloudSnapshot() {
    ...
    const { data, error } = await supabase.from(TABLE)
      .select('id,state,state_version,source_version,created_at')
  ...
  async function restoreLatestCloudSnapshot(options = {}) {
    ...
    if (currentRaw && !options.overwriteExisting) {
      return { ok: false, reason: 'local_state_exists', snapshot: result.snapshot };
    }
  ...
  window.medcontrolCloudSnapshot = {
    create: createCloudSnapshot,
    latest: getLatestCloudSnapshot,
    restoreLatest: restoreLatestCloudSnapshot,
    validState,
    meaningfulState
  };
})();
