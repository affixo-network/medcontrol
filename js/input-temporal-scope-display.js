(function(){
  function scopeFor(med){const app=med&&med.temporalApplication,today=typeof currentLocalDate==='function'?currentLocalDate():'';if(!app||app.date!==today)return null;if(!['today','future','today_future'].includes(app.scope))return null;return app;}
  function findRowForMedication(med){const rows=[...document.querySelectorAll('table tbody tr')];return rows.find(row=>{const cells=row.querySelectorAll('td');if(cells.length<11)return false;const order=Number((cells[0].textContent||'').trim()),name=(cells[1].textContent||'').replace(/\s+/g,' ').trim();return Number(med.order)===order&&name.includes(String(med.name||'').replace(/\s+/g,' ').trim());})||null;}
  function diffTimes(a,b){const other=new Set(Array.isArray(b)?b:[]);return (Array.isArray(a)?a:[]).filter(t=>!other.has(t));}
  ...
  const originalMount=window.mount;if(typeof originalMount==='function'){window.mount=function(page){const result=originalMount(page);if(page==='input'){setTimeout(updateInputScopeDisplay,0);setTimeout(updateInputScopeDisplay,100);}return result;};}
  const observer=new MutationObserver(()=>scheduleRefresh());observer.observe(document.documentElement,{childList:true,subtree:true});window.addEventListener('load',()=>setTimeout(updateInputScopeDisplay,0));window.addEventListener('focus',()=>setTimeout(updateInputScopeDisplay,0));window.updateInputScopeDisplay=updateInputScopeDisplay;
})();
