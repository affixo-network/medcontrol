(function(){
  ...
  function openMedicationEditor(id){
    if(typeof window.openEditMedication==='function'){window.openEditMedication(id);return;}
    if(typeof openEditMedication==='function'){openEditMedication(id);return;}
    alert('Редактор препарата недоступен.');
  }
  ...
  window.editMedicationGeneral=function(id){openMedicationEditor(id);};
  ...
  function restructureInputTable(){
    ...
    document.querySelectorAll('section').forEach(section=>{
      ...
      [...tbody.querySelectorAll('tr')].filter(r=>!r.dataset.timeSubrow).forEach(baseRow=>{
        const cells=baseRow.querySelectorAll('td');if(cells.length<15)return;const med=byOrder.get(cells[0].textContent.trim());if(!med)return;cells[10].textContent='—';cells[13].textContent='—';cells[14].innerHTML=`<div class="inline"><button type="button" onclick="editMedicationGeneral('${med.id}')">Изменить</button><button type="button" onclick="showRowHistory('${med.id}')">История</button><button type="button" onclick="startMedicationCancellation('${med.id}')">Отменить</button></div>`;let anchor=baseRow;
