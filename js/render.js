  ...
  const body = `
    ...
    <section class="grid2">
      ...
      <div class="card">
        ...
        <div class="help"><ul>
          <li>Отметьте часы и минуты, затем нажмите «Добавить время».</li>
          <li>Для дней недели поставьте отметки напротив нужных дней.</li>
          <li>Для отдельных дат отметьте дату в календаре и добавьте её.</li>
          <li>Если обязательное значение пропущено, приложение покажет подсказку.</li>
        </ul></div>
      </div>
    </section>
    ...
    <dialog id="medicationConfirmDialog"><h2>Подтверждение препарата</h2><div id="medicationConfirmContent"></div><div class="dialog-actions"><button type="button" onclick="cancelMedicationCreate()">Вернуться к редактированию</button><button type="button" onclick="confirmMedicationCreate()">Подтвердить и сохранить</button></div></dialog>`;
