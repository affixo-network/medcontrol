...
function getState() {
  ...
  if (raw) {
    try {
      ...
      parsed.intakeCorrections = Array.isArray(parsed.intakeCorrections) ? parsed.intakeCorrections : [];
      if (!parsed.settings.interfaceLanguage) parsed.settings.interfaceLanguage = 'en';
      if (!TRANSLATIONS[parsed.settings.interfaceLanguage]) parsed.settings.interfaceLanguage = 'en';
      ...
      try { localStorage.setItem(STORAGE_CORRUPT_BACKUP_KEY, raw); }
      catch (backupError) { console.error('MedControl: failed to preserve corrupted storage.', backupError); throw err; }
...
function saveState(state) {
  ...
  try {
    ...
    if (window.MedControlSupabaseStorage) window.MedControlSupabaseStorage.queueSync(state);
    return true;
  } catch (error) {
    console.error('MedControl: failed to save application state.', error);
    if (typeof window !== 'undefined' && typeof window.alert === 'function') window.alert('Не удалось сохранить изменения MedControl.\n\nПредыдущие сохранённые данные не изменены. Освободите место в хранилище браузера или проверьте его доступность и повторите действие.');
    return false;
  }
}
