(function(){
  ...
  function todayTable(){
    ...
    return `<table><thead><tr><th>№</th><th>Препарат</th><th>Дата</th><th>Расчётное время</th></tr></thead><tbody>${todayOnly.map(x=>`<tr><td>${esc(x.m.order||'—')}</td><td>${esc(x.m.name||'—')}</td><td>${esc(formatDate(x.date))}</td><td>${esc(x.time)}</td></tr>`).join('')}</tbody></table>`;
  }
